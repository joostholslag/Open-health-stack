...
flowchart TB
  subgraph k8s["Kubernetes objects (namespace: health-stack)"]
    direction TB
    ing["<b>Ingress</b><br/>/fhir → hapi · /ehrbase → ehrbase · /openfhir → openfhir"]

    subgraph wl["Workloads"]
      hapi["<b>hapi</b> Deployment<br/>HAPI FHIR + interceptor JAR<br/>replicas: 1"]
...
**4 app pods + 1 database pod.** The add-ons (ingress-nginx, cert-manager, CCM, CSI)
add a handful more in `ingress-nginx`, `cert-manager` and `hcloud-system` namespaces.
...
client → DNS(health.example.com) → LB public IP
      ...
      → (IPS bundle) → openfhir Service → openfhir pod
      → ehrbase Service → ehrbase pod
